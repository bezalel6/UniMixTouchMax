...
#include "MessageMigrationAdapter.h"
#include <esp_log.h>
...
bool initMessaging() {
    ...
    if (success) {
        ESP_LOGI(TAG, "Messaging system initialized - no abstractions, just serial");
    } else {
...
String getMessagingStatus() {
    ...
    status += "- Active handlers: " + String(MessageRouter::getInstance().getHandlerCount()) + "\n";
